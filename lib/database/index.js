 ...
 * @api public
 */
Database.prototype.get = function get(collectionName, key, cb) {
    var noSqlKey = Utils.sanitize(key);

    // Find the newly created record and return it
    this.connection.get(noSqlKey, function(err, retrievedValue) {
        if(err) return next(err);
 ...
 * @api public
 */
Database.prototype.set = function set(collectionName, key, value, seconds, next) {
    var self = this;
    var noSqlKey = Utils.sanitize(key);
    var retrievedValue = {};
    ...
    function sendResults(err, results) {
        ...
        // Find the newly created record and return it
        self.connection.get(noSqlKey, function(err, retrievedValue) {
            if(err) return next(err);
    ...
    if (seconds) {
        self.connection.setex(noSqlKey, seconds, parsedData, sendResults);
    } else {
        self.connection.set(key, value, sendResults);
    }
};

/**
 * Update the TTL on a simple KV pair.
 *
 * @param {String} collectionName
 * @param {Object} key
 * @param {integer} seconds Time until expiration.
 * @param {Function} callback
 * @api public
 */
Database.prototype.updateTTL = function updateTTL(collectionName, key, seconds, cb) {
    var noSqlKey = Utils.sanitize(key);

    this.connection.expire(key, seconds, function sendResults(err, results) {
            if(err) return cb(err);
            cb(null, results);
        }
    );
