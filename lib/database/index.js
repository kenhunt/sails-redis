...
Database.prototype.find = function find(collectionName, criteria, cb) {
  ...
  if(criteria.where && Utils.present(criteria.where[primary])) {
    var noSqlKey = this.schema.recordKey(name, primary, criteria.where[primary]);
    noSqlKey = Utils.sanitize(noSqlKey);

    connection.get(noSqlKey, function(err, record) {
      if(err) return cb(err);
  ...
  async.auto({
    ...
    updateRecords: ['updateIndexes', function(next, results) {
      ...
      function updateRecord(item, nextItem) {
        ...
        // KH - I'm fairly certain key should be sanitized here. 
